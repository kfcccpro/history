...
const labels={concept:'세부 개념까지 알고 있음',branch:'큰 가지부터 다시',root:'오늘 범위부터 다시',unit:'대단원부터 다시'};
...
const html=`<section class="section"><h2>개념 뎁스 복구 · 어디부터 다시 알아야 하는가</h2><div class="desc">오답 뒤 세부 개념에서 시작해 모르면 상위 가지로 올라가고, 마지막으로 연결되는 지점부터 다시 내려온 기록입니다. 최근 7일 ${x.recent7}회.</div><div class="depth-parent-bars">${rows}</div>${branches?`<div class="depth-parent-list">${branches}</div>`:''}</section>`;
