(function(){
  ...
  function statusText(s){
    ...
    if(s.protocol==='file:')return 'Firebase Hosting 배포 후 기기 간 동기화가 활성화됩니다.';
  ...
  function render(){const el=document.getElementById('firebaseSyncBody'),api=window.History2CloudSync||{},s=api.state||{};if(!el)return;document.getElementById('firebaseSyncStatus').textContent=statusText(s);
    ...
    if(s.protocol==='file:'){el.innerHTML='<p class="desc">로컬 파일에서는 기존 기기 저장을 사용합니다. Firebase Hosting 주소로 접속하면 학생 로그인 없이 자동 동기화됩니다.</p>';return}
