...
let wrapped=false,markedWrongForAttempt=false,finishing=false;
...
async function locator(){
  ...
  const registry=window.H2WrongAnswerRegistry;let rec=registry&&gateUid?registry.get(gateUid):null;
...
async function markWrong(){if(markedWrongForAttempt)return;markedWrongForAttempt=true;try{if(window.H2WrongAnswerRegistry&&gateUid)H2WrongAnswerRegistry.markGateAttempt(gateUid,false,{bookUsed:false})}catch(_){}}
async function markCorrect(){if(finishing)return;finishing=true;try{if(window.H2WrongAnswerRegistry&&gateUid)H2WrongAnswerRegistry.markGateAttempt(gateUid,true,{bookUsed:markedWrongForAttempt})}catch(_){}}
async function renderLookup(){
  ...
  const b=document.getElementById('h2BookRetry');if(b)b.onclick=()=>{markedWrongForAttempt=false;try{if(typeof resetRuntimeForQuestion==='function')resetRuntimeForQuestion();else{runtime.mode='question';runtime.answer=null;runtime.feedbackStep=0;runtime.lastDiagnosis=null}if(typeof saveState==='function')saveState();baseRender()}catch(_){location.reload()}};
}
...
function install(){
  ...
  try{
    ...
    if(requested&&Array.isArray(COURSE.problems)){
      ...
      if(i>=0&&state.problemIndex!==i){state.phase='learn';state.problemIndex=i;try{if(typeof resetRuntimeForQuestion==='function')resetRuntimeForQuestion();else{runtime.mode='question';runtime.answer=null;runtime.feedbackStep=0;runtime.lastDiagnosis=null}if(typeof saveState==='function')saveState(false)}catch(_){}}
    }
    baseRender=render;
    render=function(){
